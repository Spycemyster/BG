...
func _download(ref: StorageReference, meta_only: bool, url_only: bool) -> StorageTask:
	...
	print(str(task.action) + " " + str(StorageTask.Task.TASK_DOWNLOAD));
	_pending_tasks.append(task)

	await info_task.task_finished
	if info_task.data != null and not info_task.data.has("error"):
...
func _finish_request(result: int) -> void:
	...
	match task.action:
		...
		StorageTask.Task.TASK_DOWNLOAD_URL:
			...
			test_json_conv.parse(_response_data.get_string_from_utf8())
