...
func _on_request_completed(result : int, response_code : int, headers : PackedStringArray, body : PackedByteArray) -> void:
	var bod
	if JSON.parse_string(body.get_string_from_utf8()) == OK:
	...
	match action:
		...
		Task.TASK_PATCH:
			if offline:
				...
				if offline:
					...
					test_json_conv2.parse(_fields)
					mod = {
						"name": "projects/%s/databases/(default)/documents/%s" % [Firebase._config["storageBucket"], data],
						"fields": test_json_conv2.get_data(),
						"createTime": "from_cache_file",
						"updateTime": "from_cache_file"
					}
		...
		Task.TASK_LIST:
			if offline:
				...
				cache_dir.remove_at(cache_path.path_join(Firebase.Firestore._CACHE_RECORD_FILE))
				print(cache_files)
