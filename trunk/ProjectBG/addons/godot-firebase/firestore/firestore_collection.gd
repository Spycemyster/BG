...
func get(document_id : StringName) -> Variant:
	var document_id_str = String(document_id)
	var task : FirestoreTask = FirestoreTask.new()
	task.action = FirestoreTask.Task.TASK_GET
	task.data = collection_name + "/" + document_id_str
	var url = _get_request_url() + _separator + document_id_str.replace(" ", "%20")
