...
var networking: bool = true :
	get:
		return networking # TODOConverter40 Non existent get function 
	set(mod_value):
		mod_value  # TODOConverter40 Copy here content of set_networking
...
var _offline: bool = false :
	get:
		return _offline # TODOConverter40 Non existent get function 
	set(mod_value):
		mod_value  # TODOConverter40 Copy here content of _set_offline
...
func _set_offline(value: bool) -> void:
	...
	if not value:
		...
		if file.open_encrypted_with_pass(event_record_path, File.READ, _encrypt_key) == OK:
			offline_time = file.get_buffer(file.get_length()).get_string_from_utf8().to_int() - 2
		file.close()

		var cache_dir := Directory.new()
		var cache_files := []
		if cache_dir.open(_cache_loc) == OK:
			cache_dir.list_dir_begin() # TODOGODOT4 fill missing arguments https://github.com/godotengine/godot/pull/40547
			var file_name = cache_dir.get_next()
			while file_name != "":
				if not cache_dir.current_is_dir() and file_name.ends_with(_CACHE_EXTENSION):
					if file.get_modified_time(_cache_loc.path_join(file_name)) >= offline_time:
						cache_files.append(_cache_loc.path_join(file_name))
#                    else:
#                        print("%s is old! It's time is %d, but the time offline was %d." % [file_name, file.get_modified_time(_cache_loc.plus_file(file_name)), offline_time])
		...
		cache_dir.remove_at(event_record_path)
...
func _set_config(config_json : Dictionary) -> void:
	...
	var file := File.new()
	if file.file_exists(_cache_loc.path_join(_CACHE_RECORD_FILE)):
		_offline = true
	else:
		_offline = false
...
func _pooled_request(task : FirestoreTask) -> void:
	...
	if auth == null and not Firebase.emulating:
